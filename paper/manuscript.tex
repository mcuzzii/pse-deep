...
\newpage
\phantomsection
\addcontentsline{toc}{section}{Annex 1: Declaration of Authenticity by Thesis Writer}
\includepdf[pages=-, pagecommand={\thispagestyle{plain}}]{Annex 1 Declaration of Authenticity by Thesis Writer.pdf}

\newpage
\phantomsection
\addcontentsline{toc}{section}{Annex 2: Authorization by Author}
\includepdf[pages=-, pagecommand={\thispagestyle{plain}}]{Annex 2 Authorization by Author.pdf}

\newpage
\phantomsection
\addcontentsline{toc}{section}{Annex 3: Terms of Use}
\includepdf[pages=-, pagecommand={\thispagestyle{plain}}]{Annex 3 Terms of Use.pdf}
...
Table \ref{tab:social_media_data} showcases the relevant columns found in the raw social media datasets. The variables RetweetCount, ReplyCount, LikeCount, QuoteCount, ViewCount, and BookmarkCount are the \textit{post impact features}, whereas the variables prefixed by ``Author'' are the \textit{influence features}. The current source for social media data is the Twitter Scraper V2 Apify Actor, which is an online pre-built web scraper with event-based pricing. The total dataset size is to be reported in the next chapter.
...
Throughout, tensors are written with explicit index sets to make batching unambiguous. The following items are either model design hyperparameters (when values are specified) whose values are \textit{the same} across all experimental settings unless otherwise specified in the following, or intermediately derived tensor dimension sizes.
...
\begin{itemize}
  ...
  \item Validation checkpoints (by batch number): $(8k)^2,\qquad k=1,2,...$
